...
const BusinessCard = ({ business, onSelect, onEdit, isSelected }) => {
  ...
  return (
    <div 
      ...
      className={`p-4 bg-white/90 backdrop-blur-sm border rounded-xl cursor-pointer transition-all ${
        isSelected 
          ? 'border-[#D4AF37] shadow-lg shadow-[#D4AF37]/10' 
          : 'border-[#2D7A4A]/20 hover:border-[#333]'
      }`}
    >
      ...
      <h3 className="text-base font-medium text-[#1A1A2E] mb-1">{business.name}</h3>
...
const CreateBusinessModal = ({ onClose, onCreated, token }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-white/90 backdrop-blur-sm border border-[#2D7A4A]/20 rounded-xl overflow-hidden">
        <div className="p-6 border-b border-[#2D7A4A]/20">
          <h2 className="text-xl font-medium text-[#1A1A2E]">Create New Business</h2>
          <p className="text-sm text-[#666] mt-1">Set up ORA agents for a new business</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white/70 border border-[#2D7A4A]/20 rounded-lg text-[#1A1A2E] placeholder-[#555] focus:border-[#D4AF37] focus:outline-none"
              required
            />
          </div>
                ...
                <button
                  ...
                  className={`p-2 text-xs rounded-lg border transition-all ${
                    formData.type === type.id
                      ...
                      : 'bg-white/70 border-[#2D7A4A]/20 text-[#888] hover:border-[#333]'
                  }`}
                >
                  {type.label}
                </button>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-4 py-3 bg-white/70 border border-[#2D7A4A]/20 rounded-lg text-[#1A1A2E] placeholder-[#555] focus:border-[#D4AF37] focus:outline-none resize-none"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm text-[#666] hover:text-[#555] transition-colors"
            >
              Cancel
            </button>
            <button
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-medium text-[#1A1A2E]">Business Management</h1>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-4">
          {businessDetails ? (
            <>
              {/* Business Info */}
              <div className="p-6 bg-white/90 backdrop-blur-sm border border-[#2D7A4A]/20 rounded-xl">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h2 className="text-xl font-medium text-[#1A1A2E]">{businessDetails.business?.name}</h2>
                ...
                <div className="grid grid-cols-3 gap-4 pt-4 border-t border-[#2D7A4A]/20">
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-semibold text-[#1A1A2E]">OODA</div>

...
Handles email offer campaigns via Gmail (DIY — no Resend costs).
...
def send_email(to_email: str, subject: str, html_content: str, name: str = None):
    """Send email via Gmail (DIY) - can be imported and used elsewhere"""
    from routers.email_service import send_email as central_send
    return central_send(to_email, subject, html_content)
...
async def send_offer(payload: SendOfferRequest, request: Request):
    """
    Send a marketing offer email to subscribers via Gmail (DIY).
    ...
    for user in recipients:
        try:
            ...
            # Send via Gmail DIY
            success = send_email(user.get("email"), personalized_subject, html_content)
            if success:
                sent_count += 1
                logger.info(f"Offer sent to {user.get('email')}")
            else:
                skipped_count += 1
                errors.append({"email": user.get("email"), "error": "Gmail send returned false"})
    ...
    return JSONResponse({
        ...
        "message": f"Offer sent to {sent_count} subscribers via Gmail",

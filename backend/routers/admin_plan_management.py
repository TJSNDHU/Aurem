...
from routers.admin_mission_control_router import verify_admin as _legacy_verify
...
# ═══════════════════════════════════════════════════════════════════════════════
# PLAN ASSIGNMENT & USAGE MONITORING
# ═══════════════════════════════════════════════════════════════════════════════

@router.post("/assign")
async def assign_plan_to_tenant(body: dict, admin=Depends(verify_admin)):
    ...
    if not tenant_id or not tier:
        raise HTTPException(400, "tenant_id and tier required")

    plan = await _db.subscription_plans.find_one({"tier": tier, "active": True})
    if not plan:
        raise HTTPException(404, f"Plan tier '{tier}' not found")

    await _db.aurem_workspaces.update_many(
        {"tenant_id": tenant_id},
        {"$set": {"tier": tier, "plan": tier, "plan_updated_at": datetime.now(timezone.utc).isoformat()}}
    )
    ...
    await _db.aurem_audit_logs.insert_one({
        "action": "plan_assigned", "tenant_id": tenant_id,
        "tier": tier, "admin_id": admin,
        "timestamp": datetime.now(timezone.utc).isoformat(),
    })

    logger.info(f"[Plans] Assigned {tier} to {tenant_id}")
    return {"status": "assigned", "tenant_id": tenant_id, "tier": tier}
...
@router.get("/usage-overview/all")
async def get_all_tenant_usage(admin=Depends(verify_admin)):
    ...
    for tid in tenants:
        if tid:
            ...
            total_revenue += summary.get("price_monthly", 0)

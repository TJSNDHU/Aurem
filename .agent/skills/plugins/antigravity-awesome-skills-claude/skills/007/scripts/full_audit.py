...
_BLUE_TEAM_TEMPLATES: dict[str, dict] = {
    ...
    "xss": {
        "recommendation": (
            "Use textContent for safe text insertion instead of innerHTML or "
            "dangerouslySetInnerHTML. Implement Content Security Policy (CSP) "
            "headers. Use template engines with auto-escaping (Jinja2 with "
            "autoescape, React JSX). Sanitize user HTML with DOMPurify or bleach."
        ),
        "priority": "HIGH",
        "effort": "Low to Medium",
    },

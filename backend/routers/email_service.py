"""
Central Email Service - DIY Gmail Integration
Uses tenant's connected Gmail account via GmailService (OAuth).
No external email API costs — uses Google's free SMTP quota.
"""
import os
import logging
...
def send_email(to: str, subject: str, html: str, business_id: str = None) -> bool:
    """
    Send email via Gmail (DIY — no Resend).
    Falls back to logging if Gmail is not connected.
    """
    import asyncio
    ...
    async def _send():
        ...
        if not gmail:
            logger.warning("GmailService not available - email logged only")
        ...
        if result.get("success") or result.get("message_id"):
            ...
            logger.warning(f"Gmail send failed ({error}) - email logged only")
            logger.info(f"[EMAIL LOG] To: {to} | Subject: {subject}")
            return False
...
# (send_email is now defined above using Gmail DIY)

...
def validate_all_secrets() -> Dict[str, any]:
    """
    Validate all secrets on startup. Idempotent — only runs once.
    """
    global _secrets_validated
    if _secrets_validated:
        return {"valid": True, "missing_required": [], "missing_important": []}
                        ...
                        if line and "is None" not in line and "is not None" not in line:
                            ...
                            if not line.strip().startswith('#') and "r'" not in line and 'r"' not in line:
                                ...
                                if "secrets.py" not in line:
                                    # Skip false positives: db_user, db_result, boolean flags, string literals
                                    if "db_user" not in line and "db_result" not in line and "db_record" not in line:
                                        if "_db_init" not in line and "auto_repair.py" not in line:
                                            if "'search'" not in line and "'secondary_pattern'" not in line:
                                                # Skip variable names like db_ref, db_circuit_breaker, etc.
                                                if "db_circuit" not in line and "db_ref" not in line:
                                                    # Skip docstrings and f-strings and comments
                                                    if '"""' not in line and "f'" not in line and 'f"' not in line:
                                                        # Extract just the code part after the file:line: prefix

...
## 2026-06-13 — P0: wiring audit 100% + suite signal restored + chronic restart loop killed
- Wiring audit: 67/67 (100%) — 23 probe paths corrected (admin+customer), self-probe recursion fixed, 405→wired for POST routes.
- ROOT CAUSE FIXED (chronic, 3,400+ historic restarts): auto_repair `backend_down` known-fix + AI `restart` fix restarted a HEALTHY backend on stale "Connection refused" log lines → infinite self-restart loop. Now restart only when health actually down. auto_heal: 3-fail streak for health check + streak/30-min-cooldown for missing-scheduler restart. aurem-watchdog threshold 8→20, grace 45→120s. Zero self-restarts during final 28-min suite run (verified via /usr/local/bin/supervisorctl audit wrapper — REMOVE wrapper when done debugging).
- Credentials fix: previous secrets-scrub had replaced passwords in 97 test files with literal "<REDACTED>" → mass auth failures + lockout cascades. Tests now read AUREM_ADMIN_PASSWORD/AUREM_CUSTOMER_PASSWORD, hydrated by conftest from memory/test_credentials.md. Session-start lockout clear added to conftest.

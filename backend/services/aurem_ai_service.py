...
class AuremIntelligence:
    ...
    def __init__(self, db=None):
        ...
        # iter 279 — session_id → owner user_id mapping for tenant isolation.
        # Prevents user B from resuming user A's session even with a leaked
        # session_id. Owner is recorded on first turn and checked on every
        # subsequent turn.
...
Be concise but thorough. Focus on business outcomes."""
    ...
    async def _check_session_owner(self, session_id: str, user_id: str) -> bool:
        """iter 279 — verify session_id belongs to user_id.

        Returns True if:
          - session is new (first turn, owner not yet recorded), OR
          - session owner matches user_id.
        Returns False (denies resume) if session owner exists and differs.
        Also bootstraps the in-memory owner map from DB so cross-pod sessions
        remain protected.
        """
        if not user_id:
            # Anonymous — cannot claim ownership of an existing session
            if session_id in self.session_owners:
                return False
            return True

        # In-memory fast path
        recorded = self.session_owners.get(session_id)
        if recorded is not None:
            return recorded == user_id

        # Cold-cache path — check DB for prior turns
        if self.db is not None:
            try:
                prior = await self.db.aurem_conversations.find_one(
                    {"session_id": session_id},
                    {"_id": 0, "user_id": 1},
                )
                if prior and prior.get("user_id"):
                    prior_owner = str(prior["user_id"])
                    self.session_owners[session_id] = prior_owner
                    return prior_owner == user_id
            except Exception as e:
                logger.warning(f"session owner lookup failed: {e}")

        # No prior record — this user becomes the owner
    ...
    async def chat(self, session_id: str, message: str, user_id: str = None) -> Dict[str, Any]:
        ...
        try:
            # iter 279 — tenant isolation: refuse cross-user session resume
            if not await self._check_session_owner(session_id, user_id):
                logger.warning(
                    f"[chat] session_id={session_id[:8]}… resume denied for user {user_id}"
                )
                return {
                    "response": (
                        "This chat session belongs to a different account. "
                        "Starting a fresh session for you."
                    ),
...
def get_aurem_service(db=None) -> AuremIntelligence:
    ...
    elif db is not None and _aurem_instance.db is None:

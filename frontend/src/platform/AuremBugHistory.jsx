            ...
            <button
              ...
              className={`px-3 py-1.5 rounded text-xs tracking-wide transition-all ${
                filter === f
                  ...
                  : 'bg-white/80 backdrop-blur-sm text-[#555] border border-[#151515] hover:border-[#333]'
              }`}
            >
...
const HistoryEntry = ({ entry, expanded, onToggle }) => {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      className="bg-white/80 backdrop-blur-sm border border-[#151515] rounded-lg overflow-hidden"
    >
      {/* Header */}
      <button
        onClick={onToggle}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {entry.health_checks?.map((check, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-2 bg-white/60 rounded border border-[#151515]"
                    >
                  ...
                  <div className="space-y-2">
                    {entry.ai_suggested_fixes.map((fix, i) => (
                      ...
                      >
                        ...
                        <pre className="text-xs text-[#888] bg-white/60 p-2 rounded overflow-x-auto">
                          {fix.ai_suggestion}
                        </pre>
                      </div>
                    ))}

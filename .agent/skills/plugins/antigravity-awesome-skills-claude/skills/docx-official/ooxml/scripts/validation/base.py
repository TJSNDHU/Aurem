...
class BaseSchemaValidator:
    ...
    SCHEMA_MAPPINGS = {
        ...
        "core.xml": "ecma/fouth-edition/opc-contentProperties.xsd",
    ...
    @staticmethod
    def _get_element_id(elem, attr_name):
        """Look for the specified attribute on an element and return its value."""
    ...
    def _check_global_id(
        self, id_value, tag, elem, xml_file, global_ids, errors
    ):
        """Check global uniqueness of an ID and record it."""
    ...
    @staticmethod
    def _check_file_id(
        id_value, tag, attr_name, elem, file_ids, xml_file, errors
    ):
        """Check file-level uniqueness of an ID and record it."""
        key = (tag, attr_name)
    ...
    def _validate_unique_ids_in_file(self, xml_file, global_ids, errors):
        """Validate ID uniqueness for a single XML file."""
        try:
            root = lxml.etree.parse(str(xml_file)).getroot()
            file_ids = {}  # Track IDs that must be unique within this file

            # Remove all mc:AlternateContent elements from the tree
            self._remove_alternate_content(root)
            ...
            for elem in root.iter():
                ...
                if tag in self.UNIQUE_ID_REQUIREMENTS:
                    ...
                    id_value = self._get_element_id(elem, attr_name)

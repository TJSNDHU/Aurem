...
def _find_matching_verdict(score: float) -> dict | None:
    """Return the first verdict whose [min, max] range contains ``score``.

    Returns ``None`` when no threshold matches.
    """
...
def get_verdict(score: float) -> dict:
    ...
    matched = _find_matching_verdict(clamped)

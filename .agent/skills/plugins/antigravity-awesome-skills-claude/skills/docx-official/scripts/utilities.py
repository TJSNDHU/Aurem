...
class XMLEditor:
    ...
    def _find_matching_nodes(self, tag, attrs, line_number, contains):
        """
        Find all DOM elements matching the given filters.

        Args:
            tag: The XML tag name
            attrs: Dictionary of attribute name-value pairs to match, or None
            line_number: Line number (int) or line range (range), or None
            contains: Text string that must appear in element text, or None
    ...
    def _build_not_found_error(self, tag, attrs, line_number, contains):
        """Build a descriptive error message for a not-found node."""
        filters = []

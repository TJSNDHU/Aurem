...
def _read_dep_file(filepath: Path, verbose: bool = False) -> str | None:
    """Read a dependency file, returning None on error."""
    try:
...
def _check_risky_package(pkg_name: str, file_str: str, line_num: int) -> list[dict]:
    """Return findings for risky Python packages."""
...
def _add_summary_findings(
    findings: list[dict],
    file_str: str,
    deps_total: int,
    deps_hashed: int,
) -> None:
    """Append summary-level findings for hash verification and complexity."""
...
def analyze_requirements_txt(filepath: Path, verbose: bool = False) -> dict:
    ...
    text = _read_dep_file(filepath, verbose=verbose)
    if text is None:
        return {
    ...
    for line_num, raw_line in enumerate(text.splitlines(), start=1):
        ...
        findings.extend(_check_risky_package(pkg_name, file_str, line_num))

    _add_summary_findings(findings, file_str, deps_total, deps_hashed)
...
def analyze_pipfile(filepath: Path, verbose: bool = False) -> dict:
    ...
    Returns:
        Dict with keys: deps_total, deps_pinned
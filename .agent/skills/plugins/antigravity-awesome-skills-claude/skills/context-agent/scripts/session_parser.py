...
def _parse_queue_operation(raw: dict) -> Optional[SessionEntry]:
    """Converte uma entrada do tipo queue-operation em SessionEntry."""
    return SessionEntry(
        type="queue",
...
def _parse_token_usage(usage: dict) -> dict:
    """Extrai informações de uso de tokens da mensagem."""
    if not usage:
        return {}
    return {
...
def _parse_raw_entry(raw: dict) -> Optional[SessionEntry]:
    ...
    text_parts, tool_calls, files_modified = _parse_content_blocks(content)

...
import { 
  ...
  Bug, Shield, Download, Copy, ShoppingBag, Layers
} from 'lucide-react';
...
const PlatformDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-white/60 text-[#1A1A2E] flex" style={{ fontFamily: "'Inter', sans-serif" }}>
      {/* Sidebar */}
      <motion.aside 
        initial={{ x: -20, opacity: 0 }}
        animate={{ x: 0, opacity: 1 }}
        className="w-72 bg-white/80 backdrop-blur-sm border-r border-[#151515] flex flex-col"
      >
      ...
      <main className="flex-1 overflow-y-auto flex flex-col">
        ...
        <motion.header 
          ...
          className="sticky top-0 z-10 bg-white/60/95 backdrop-blur-xl border-b border-[#151515] px-8 py-5"
        >
          ...
          <AnimatePresence mode="wait">
                    ...
                    {Object.entries(vanguardAgents).map(([id, agent]) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="p-5 bg-white/80 backdrop-blur-sm rounded-lg border border-[#151515] transition-all"
                        >
                          <div className="flex items-center justify-between mb-3">
                            <div 
            ...
            {activeTab === 'tools' && (
              ...
              >
                {/* ═══ 1-CLICK INSTALLERS ═══ */}
                <div>
                  <h3 className="text-xs font-bold tracking-[0.2em] text-[#C9A86C] mb-3 uppercase">1-Click Installers</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* WordPress Plugin */}
                    <motion.div
                      whileHover={{ y: -2, boxShadow: '0 8px 24px rgba(139,92,246,0.12)' }}
                      className="p-6 bg-gradient-to-br from-[#8B5CF6]/5 to-white/80 backdrop-blur-sm rounded-lg border border-[#8B5CF6]/20 transition-all"
                      data-testid="wordpress-plugin-card"
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[#666]">Pixel + Friend Scanner · v1.0.0</p>
                          </div>
                        </div>
                        <span className="text-[9px] font-bold tracking-[0.15em] px-2 py-1 rounded-full bg-[#4ADE80]/10 text-[#4ADE80] border border-[#4ADE80]/30">READY</span>
                      </div>
                      <p className="text-xs text-[#555] mb-4 leading-relaxed">
                        Install the AUREM tracking pixel + viral Friend Scanner on any WordPress site in under 30 seconds. No code required.
                      </p>
                      {user?.business_id && (
                        <div className="mb-3 p-3 bg-white/60 rounded border border-[#8B5CF6]/15">
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex-1 min-w-0">
                              <div className="text-[9px] tracking-[0.15em] text-[#666] mb-1">YOUR TENANT ID</div>
                              <div className="font-mono text-xs text-[#8B5CF6] truncate">{user.business_id}</div>
                            </div>
                            <button
                              type="button"
                              data-testid="wordpress-tenant-copy"
                              onClick={() => {
                                if (navigator.clipboard) {
                                  navigator.clipboard.writeText(user.business_id);
                                  const el = document.getElementById('wp-copy-feedback');
                      ...
                      <a
                        href={`${API_URL}/api/plugins/wordpress`}
                        download="aurem-pixel.zip"
                      ...
                      <details className="mt-3">
                        <summary className="text-[11px] text-[#8B5CF6] cursor-pointer hover:underline">3-step install guide</summary>
                        <ol className="text-[11px] text-[#555] mt-2 pl-4 space-y-1 list-decimal">
                          <li>Download the ZIP (above) and upload at <span className="font-mono text-[10px]">Plugins → Add New → Upload Plugin</span></li>
                          <li>Activate AUREM Pixel &amp; Intelligence</li>
                          <li>Go to <span className="font-mono text-[10px]">Settings → AUREM</span>, paste your Tenant ID, click Save Connection</li>
                        </ol>
                      </details>
                    </motion.div>
                    ...
                    {/* Shopify OAuth */}
                    <motion.div
                      whileHover={{ y: -2, boxShadow: '0 8px 24px rgba(0,152,116,0.12)' }}
                      className="p-6 bg-gradient-to-br from-[#009874]/5 to-white/80 backdrop-blur-sm rounded-lg border border-[#009874]/20 transition-all"
                      data-testid="shopify-card"
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[#666]">1-click OAuth · Pixel + Pulse</p>
                          </div>
                        </div>
                        <span className="text-[9px] font-bold tracking-[0.15em] px-2 py-1 rounded-full bg-[#4ADE80]/10 text-[#4ADE80] border border-[#4ADE80]/30">LIVE</span>
                      </div>
                      <p className="text-xs text-[#555] mb-4 leading-relaxed">
                        Connect any Shopify store in 2 clicks. Auto-installs AUREM pixel, syncs products, and enables abandoned cart recovery.
                      </p>
                      <input
                        type="text"
                        ...
                        className="w-full px-3 py-2 mb-3 border border-[#009874]/20 rounded text-sm focus:outline-none focus:border-[#009874] bg-white/60"
                      />
                      <button
                        type="button"
                        data-testid="shopify-install-btn"
                        onClick={() => {
                          const v = (document.getElementById('shopify-store-input') || {}).value || '';
                          const shop = v.trim().replace(/^https?:\/\//, '').replace(/\/$/, '');
                          if (!shop || !shop.includes('myshopify.com')) {
                            alert('Please enter a valid Shopify store URL (e.g. your-store.myshopify.com)');
                            return;
                          }
                          window.location.href = `${API_URL}/api/shopify/auth?shop=${encodeURIComponent(shop)}`;
                        }}
                ...
                {/* ═══ CONNECTED CHANNELS ═══ */}
                <div>
                  <h3 className="text-xs font-bold tracking-[0.2em] text-[#C9A86C] mb-3 uppercase">Connected Channels</h3>
                      ...
                      return (
                        ...
                        >
                          ...
                          <button className={`w-full py-2.5 rounded text-sm tracking-wide transition-all ${
                            status?.connected
                              ? 'bg-white/50 text-[#555] hover:bg-[#1A1A1A]'
            ...
            {activeTab === 'api' && (
              ...
              >
                <div className="p-6 bg-white/80 backdrop-blur-sm rounded-lg border border-[#151515]">
                  ...
                  <div className="p-4 bg-white/60 rounded border border-[#151515] font-mono text-sm text-[#D4AF37] mb-4">
                    {user?.api_key || 'Loading...'}
                  </div>
                  <p className="text-xs text-[#444]">
                    Header: <code className="bg-white/50 px-2 py-1 rounded text-[#D4AF37]">X-API-Key</code>
                  </p>
                </div>

                <div className="p-6 bg-white/80 backdrop-blur-sm rounded-lg border border-[#151515]">
                  <h3 className="text-sm font-medium mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>Launch Vanguard via API</h3>
                  <pre className="p-4 bg-white/60 rounded border border-[#151515] text-xs overflow-x-auto text-[#666]">
  ...
  <motion.div 
    ...
    className={`p-5 rounded-lg border transition-all ${
      highlight 
        ...
        : 'bg-white/80 backdrop-blur-sm border-[#151515]'
    }`}
  >
    <div className="flex items-center justify-between mb-3">
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {expanded && log.data && Object.keys(log.data).length > 0 && (
          <div className="mt-2 p-2 bg-white/80 backdrop-blur-sm rounded text-[10px] text-[#555]">
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`px-2 py-1 rounded border text-[10px] tracking-wider uppercase ${styles[status] || 'bg-white/50 text-[#444]'}`}>
      {status}
    </span>
  );
};

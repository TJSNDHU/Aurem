...
# Regex patterns for Pipfile parsing
_PIPFILE_SECTION_RE = re.compile(r"""^\s*\[""")
_PIPFILE_DEPS_SECTION_RE = re.compile(
    r"""^\[(?:packages|dev-packages)\]""", re.IGNORECASE
)
_PIPFILE_PKG_RE = re.compile(
    r"""^([A-Za-z0-9_][A-Za-z0-9._-]*)\s*=\s*['"]([^'"]*)['\"]"""
)
_PIPFILE_DICT_RE = re.compile(
    r"""^([A-Za-z0-9_][A-Za-z0-9._-]*)\s*=\s*\{"""
)


def _process_pipfile_string_dep(
    pkg_name: str,
    version_spec: str,
    line_num: int,
    file_str: str,
) -> tuple[bool, list[dict]]:
    """Process a Pipfile string-style dependency line.

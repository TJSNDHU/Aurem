...
async def _collect_results(estados_alvo: List[str], concurrency: int) -> List[dict]:
    """Executa o scraping de todos os estados em paralelo."""
...
def _persist_results(results: List[dict]) -> tuple:
    """Persiste resultados no banco e retorna (total_coletados, total_salvos, log_entries)."""
    db = Database()
    db.init()
...
async def run(estados: Optional[List[str]], concurrency: int, dry_run: bool) -> None:
    ...
    results = await _collect_results(estados_alvo, concurrency)
    total_coletados, total_salvos, log_entries = _persist_results(results)

...
_BLUE_TEAM_TEMPLATES: dict[str, dict] = {
    ...
    "code_injection": {
        "recommendation": (
            "Remove all uses of dynamic code execution built-ins and the "
            "Function constructor. If dynamic code execution is absolutely "
            "necessary, use a sandboxed environment (e.g. RestrictedPython, "
            "vm2 in strict mode) with allowlisted operations only. Never pass "
            "user input to code execution functions."
        ),
        "priority": "CRITICAL",
        "effort": "Medium",
    },

...
def get_current_user(authorization: str = Header(None)):
    ...
    try:
        ...
        # iter 282al-19 — silence stderr flood in production. Stale tokens
        # are normal (clients with expired sessions); they should not fill
        # the pod logs and create the impression of a deploy failure.
        import logging as _log
        _log.getLogger(__name__).debug(f"[Auth] token verify failed: {e}")
...
async def generate_api_key(authorization: str = Header(None)):
    ...
    try:
        ...
        tenant_id = user.get("tenant_id") or user.get("id") or user_id
...
async def validate_api_key(api_key: str) -> dict:
    """Validate API key, enforce daily rate limit (1000/day), return tenant info."""
    try:
        from server import db
        ...
        # Daily rate limit: 1000 calls per API key per day
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        daily_key = f"daily_{key_doc['key_id']}_{today}"
        usage_doc = await db.api_key_daily_usage.find_one(
            {"key": daily_key}, {"_id": 0}
        )
        daily_count = usage_doc.get("count", 0) if usage_doc else 0
        
        if daily_count >= 1000:
            return {"error": "daily_limit_exceeded", "limit": 1000, "used": daily_count}
        
        # Increment usage counters
        await db.api_key_daily_usage.update_one(
            {"key": daily_key},
            {"$inc": {"count": 1}, "$set": {"date": today, "key_id": key_doc["key_id"]}},
            upsert=True,
        )
        await db.api_keys.update_one(

...
class AuremBugEngine:
    ...
    async def _get_ai_suggestion(self, error: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # Hard 12s cap — prevents event-loop starvation if Emergent 502s
            # during rate-limit cascades.
            response = await asyncio.wait_for(
                chat.send_message(UserMessage(text=f"Fix this error:\n{error_text}")),
                timeout=12.0,
            )
            ...
            logger.warning("[BUG ENGINE] AI suggestion timed out — skipping")
            return None
        except Exception as e:

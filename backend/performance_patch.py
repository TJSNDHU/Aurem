...
class CacheManager:
    ...
    async def connect(self):
        """Alias for connect_redis — called during startup."""
        return await self.connect_redis()

    async def disconnect(self):
        """Release shared pool reference (pool itself closed by utils.redis_pool)."""
        self._redis_connected = False
        self._redis = None

    @property
    def available(self):
        """Whether the cache backend is functional (always True — in-memory fallback)."""
        return True
    ...
    async def connect_redis(self):
        ...
        try:
            ...
            logger.info("✓ CacheManager using shared Redis pool")
            return True
        except Exception as e:
    ...
    def _generate_key(self, prefix: str, *args, **kwargs) -> str:
        ...
        if kwargs:
            key_data += f":{hashlib.sha256(str(sorted(kwargs.items())).encode()).hexdigest()[:8]}"
        return key_data
...
def cached(cache_manager: CacheManager, prefix: str, ttl: int = 300):
    ...
    def decorator(func: Callable):
        async def wrapper(*args, **kwargs):
            # Generate cache key
            cache_key = f"{prefix}:{hashlib.sha256(str(args).encode() + str(kwargs).encode()).hexdigest()[:12]}"

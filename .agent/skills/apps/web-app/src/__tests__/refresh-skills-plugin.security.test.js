...
describe('refresh-skills plugin security', () => {
  ...
  it('rejects token-less requests when refresh token is configured', async () => {
    // TODO: Set TEST_SKILLS_REFRESH_TOKEN env var to configure the test refresh token
    process.env.SKILLS_REFRESH_TOKEN = process.env.TEST_SKILLS_REFRESH_TOKEN || 'test-refresh-token';
    const handler = await loadRefreshHandler();
    const req = {
      method: 'POST',
      headers: {

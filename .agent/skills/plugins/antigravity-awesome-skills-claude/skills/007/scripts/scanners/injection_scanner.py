...
Detects code injection, SQL injection, command injection, promptinjection,
...
def _only_hardcoded_string(line: str) -> bool:
    ...
    For example, calling ``eval`` with a constant expression or
    ``os.system`` with a fixed string and no variables.
    """

...
async def receive_webhook(request: Request):
    ...
    # Adaptive ORA: surface inbound replies as conviction signals.
    # We parse the payload independently (defensive) so even if process_webhook
    # changes shape in future, the signal pipeline still works off the raw Meta
    # format.
    try:
        from services.adaptive_ora import record_signal, classify_reply_intent
        db = get_db()
        for entry in (payload or {}).get("entry", []) or []:
            for change in entry.get("changes", []) or []:
                v = change.get("value", {}) or {}
                # inbound messages live under value.messages[]; statuses under value.statuses[]
                for msg in v.get("messages", []) or []:
                    from_number = (msg.get("from") or "").replace("+", "")
                    if not from_number:
                        continue
                    ...
                    await record_signal(db, lid, "whatsapp_reply", {"text_preview": text[:120]})
                    if classify_reply_intent(text):
                        await record_signal(db, lid, "whatsapp_reply_intent", {"matched": True})
                # read receipts → whatsapp_read
                for st in v.get("statuses", []) or []:
                    if st.get("status") == "read":
                        to_number = (st.get("recipient_id") or "").replace("+", "")
                        if not to_number:
                            continue
                        lead = await db.campaign_leads.find_one(
                        ...
                        if lead and lead.get("lead_id"):
                            await record_signal(db, lead["lead_id"], "whatsapp_read")
    except Exception as e:
        logger.debug(f"[AdaptiveORA] webhook signal pipeline skipped: {e}")

...
class RedliningValidator:
    ...
    def validate(self):
        ...
        # If there are no tracked changes by Claude, nothing to validate
        if not self._has_claude_tracked_changes(modified_file):
            if self.verbose:
                print("PASSED - No tracked changes by Claude found.")
            return True

        # Unpack original docx and compare documents
        with tempfile.TemporaryDirectory() as temp_dir:
            original_file = self._unpack_original_docx(Path(temp_dir))
            if original_file is None:
                return False

            return self._compare_documents(modified_file, original_file)

    def _has_claude_tracked_changes(self, modified_file):
        """Check whether the modified document contains tracked changes by Claude."""
        try:
    ...
    def _unpack_original_docx(self, temp_path):
        """Unpack the original docx into *temp_path* and return the document.xml path.
    ...
    def _compare_documents(self, modified_file, original_file):
        """Parse both documents, remove Claude's tracked changes, and compare text."""
        try:
            import xml.etree.ElementTree as ET

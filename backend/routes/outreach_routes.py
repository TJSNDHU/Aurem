...
async def weather_test_simple():
    """Simple weather test endpoint. Uses Open-Meteo (free, no key)."""
    try:
        from services.free_api_arsenal import get_weather
        w = await get_weather(43.65, -79.38, "Toronto")
        return {
            "success": w.get("temp_c") is not None,
            "api_key_configured": True,
            "city": w.get("city", "Toronto"),
            "current_temp": w.get("temp_c"),
            "condition": w.get("condition", ""),
            "source": "open-meteo (free)",
            "alerts_active": True,
        }
    except Exception as e:
        return {"success": False, "error": str(e), "source": "open-meteo"}

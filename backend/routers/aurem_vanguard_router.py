...
async def execute_vanguard_swarm(mission_id: str, config: VanguardMission, key_info: Optional[Dict[str, Any]] = None):
    ...
    try:
        ...
        # Import WhatsApp Engine and Orchestrator
        try:
            ...
            try:
                ...
                pass
            if _wa_db:
                wa_engine = WhatsAppEngine(_wa_db)
                whatsapp_available = True
            else:
                whatsapp_available = False
                logger.warning("[AUREM] WhatsApp engine: DB not available")
        except ImportError:
            whatsapp_available = False
            wa_engine = None
            logger.warning("[AUREM] WhatsApp engine not available")
        ...
        for i, plan in enumerate(outreach_plans[:outreach_to_send]):
            ...
            if channel == "whatsapp" and whatsapp_available and phone:
                try:
                    # Route via WhatsApp Hybrid Engine (Meta Cloud → WHAPI fallback)
                    tenant_id = user_id or "polaris-built-001"
                    result = await wa_engine.send_message(tenant_id, phone, message)
                ...
                try:
                    ...
                    if _email_db:
                        ...
                        email_addr = plan.get("email", "")
                        if email_addr:
                            tenant_id_for_email = user_id or "polaris-built-001"
                            e_result = await email_engine.send_message(
                                tenant_id_for_email, email_addr,
                                f"AUREM — {company} Opportunity",
                                f"<p>Hi {plan.get('contact_name', 'there')},</p><p>{message}</p><p>— ORA, AUREM AI</p>"
                            )
                            if e_result.get("success"):
                                send_result = {"success": True, "channel": "email", "email_id": e_result.get("email_id"), "engine": "resend"}
                                outreach_sent += 1
                                await log_event("closer", f"Sent email to {company}", {"status": "delivered", "email_id": e_result.get("email_id")})
                            else:
                                send_result = {"success": False, "channel": "email", "error": e_result.get("error")}
                                await log_event("closer", f"Email failed for {company}: {e_result.get('error', '')[:50]}")
                        else:
                            send_result = {"success": False, "channel": "email", "error": "No email address"}
                    else:
                        send_result = {"success": False, "channel": "email", "error": "DB not available"}
                except Exception as e:
                    send_result = {"success": False, "channel": "email", "error": str(e)}
                    logger.error(f"[AUREM] Email send error: {e}")
                if send_result.get("success") and orchestrator_available:
                    await increment_channel_counter("email", user_id)
            
            elif channel == "sms":
                # Route via SMS Engine (Twilio)
                try:
                    from services.sms_engine import SMSEngine
                    _sms_db = None
                    try:
                        import server as _srv2
                        if hasattr(_srv2, "db") and _srv2.db is not None:
                            _sms_db = _srv2.db
                    except Exception:
                        pass
                    if _sms_db and phone:
                        sms_engine = SMSEngine(_sms_db)
                        tenant_id_for_sms = user_id or "polaris-built-001"
                        s_result = await sms_engine.send_message(tenant_id_for_sms, phone, message)
                        if s_result.get("success"):
                            send_result = {"success": True, "channel": "sms", "message_sid": s_result.get("message_sid"), "engine": "twilio"}
                            outreach_sent += 1
                            await log_event("closer", f"SMS sent to {company}", {"status": "delivered", "sid": s_result.get("message_sid")})
                        else:
                            send_result = {"success": False, "channel": "sms", "error": s_result.get("error")}
                    else:
                        send_result = {"success": False, "channel": "sms", "error": "No phone or DB unavailable"}
                except Exception as e:
                    send_result = {"success": False, "channel": "sms", "error": str(e)}
                    logger.error(f"[AUREM] SMS send error: {e}")
                if send_result.get("success") and orchestrator_available:
                    await increment_channel_counter("sms", user_id)

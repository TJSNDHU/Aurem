...
class _NoOp:
    """No-op metric that silently ignores all calls when prometheus is not installed."""
...
if PROM_AVAILABLE:
    api_requests_total = Counter('aurem_api_requests_total', 'Total API requests', ['endpoint', 'method', 'status'])
    llm_tokens_total = Counter('aurem_llm_tokens_total', 'Total LLM tokens used', ['service', 'model', 'user_tier'])
    llm_cost_total = Counter('aurem_llm_cost_usd_total', 'Total LLM cost in USD', ['service', 'model'])
    api_latency_seconds = Histogram('aurem_api_latency_seconds', 'API request latency', ['endpoint'], buckets=(0.1, 0.5, 1.0, 2.0, 5.0, 10.0))
    llm_ttft_seconds = Histogram('aurem_llm_ttft_seconds', 'Time to first token (LLM)', ['service', 'model'], buckets=(0.1, 0.3, 0.5, 1.0, 2.0, 5.0))
    llm_total_time_seconds = Histogram('aurem_llm_total_time_seconds', 'Total LLM generation time', ['service', 'model'], buckets=(0.5, 1.0, 2.0, 5.0, 10.0, 30.0))
    active_subscriptions = Gauge('aurem_active_subscriptions', 'Number of active subscriptions', ['tier'])
    mrr_usd = Gauge('aurem_mrr_usd', 'Monthly Recurring Revenue in USD')
    arr_usd = Gauge('aurem_arr_usd', 'Annual Recurring Revenue in USD')
    aurem_info = Info('aurem_system', 'AUREM system information')
    aurem_info.info({'version': '1.0.0', 'format': 'TOON', 'architecture': 'SaaS + Multi-Agent', 'python_version': '3.11'})
else:
    api_requests_total = _noop
    llm_tokens_total = _noop
    llm_cost_total = _noop
    api_latency_seconds = _noop
    llm_ttft_seconds = _noop
    llm_total_time_seconds = _noop
    active_subscriptions = _noop
    mrr_usd = _noop
    arr_usd = _noop
    aurem_info = _noop

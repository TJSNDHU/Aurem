...
Gated by env flag: VOICE_WAKE_WORD_BACKEND_ENABLED=true
Default: disabled (frontend Web Speech API handles wake-word locally).
"""
...
# Feature flag — backend wake-word pipeline is disabled by default.
# Frontend continues to use Web Speech API for local wake-word detection.
WAKE_WORD_BACKEND_ENABLED = (
    os.environ.get("VOICE_WAKE_WORD_BACKEND_ENABLED", "false").lower() == "true"
)
if not WAKE_WORD_BACKEND_ENABLED:
    logger.info("[VoiceWakeWord] Backend disabled via VOICE_WAKE_WORD_BACKEND_ENABLED=false")
...
class VoiceCommandProcessor:
    ...
    async def _get_revenue_today(self, business_id: str) -> Dict[str, Any]:
        """Get revenue for today — real Mongo aggregation on payment_transactions.

        Sums `amount` for rows where the payment landed today (UTC) and the
        status indicates a completed payment. We accept both Stripe-flavour
        statuses (`paid`, `succeeded`, `complete`, `completed`) on either
        the top-level `status` field OR `payment_status` field for backward
        compatibility with the two writers in the codebase.
        """
        ...
        if self.db is not None:
            ...
            start_yesterday = start_today - _td(days=1)
            ...
            async def _sum_window(start_iso: str, end_iso: str) -> tuple[float, str]:
                """Return (total_amount, currency_seen) over [start, end)."""
                pipeline = [
                    {
                        "$match": {
                            **paid_filter,
                ...
                cur = "USD"
                try:
                    async for row in self.db.payment_transactions.aggregate(pipeline):
                        total += float(row.get("total") or 0)
                        cur = (row.get("_id") or "usd").upper()
                except Exception as e:
                    logger.warning(f"[VoiceWakeWord] revenue aggregation failed: {e}")
                return total, cur

            try:
                revenue_today, currency = await _sum_window(
                    start_today.isoformat(), now.isoformat()
                )
                revenue_yesterday, _ = await _sum_window(
                    start_yesterday.isoformat(), start_today.isoformat()
                )
            except Exception as e:
                logger.warning(f"[VoiceWakeWord] revenue query error: {e}")

        # Honest reporting: if nothing landed today, say so. Compute % change
        # only when both sides are non-zero — otherwise skip the comparison.
        if revenue_today <= 0:
            response_text = (
                "No revenue has landed today yet. "
                f"Yesterday closed at ${revenue_yesterday:,.2f} {currency}."
                if revenue_yesterday > 0
                else "No revenue recorded today yet."
            )
            change = None
        elif revenue_yesterday > 0:
            change = round(((revenue_today - revenue_yesterday) / revenue_yesterday) * 100)
            direction = "up" if change >= 0 else "down"
            response_text = (
                f"Today's revenue is ${revenue_today:,.2f} {currency}. "
                f"You're {direction} {abs(change)}% from yesterday."
            )
        else:
            change = None
            response_text = (
                f"Today's revenue is ${revenue_today:,.2f} {currency}. "
                "No baseline from yesterday to compare against."
            )
    ...
    async def _get_latest_leads(self, business_id: str) -> Dict[str, Any]:
        """Get latest leads from database"""
        leads_cursor = self.db.leads.find(
            {"business_id": business_id, "lead_score": {"$gte": 70}},
            {"_id": 0}
        ).sort("lead_score", -1).limit(5)
        leads = await leads_cursor.to_list(length=5)
...
def get_voice_processor(db=None):
    ...
    elif db is not None and _voice_processor.db is None:

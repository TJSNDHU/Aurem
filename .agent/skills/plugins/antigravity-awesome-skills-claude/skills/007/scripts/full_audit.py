...
_BLUE_TEAM_TEMPLATES: dict[str, dict] = {
    ...
    "code_injection": {
        "recommendation": (
            "Remove all uses of dynamic code execution functions such as "
            "built-in compile-and-run helpers and dynamic function constructors. "
            "If dynamic code execution is absolutely necessary, use a sandboxed "
            "environment (e.g. RestrictedPython, vm2 in strict mode) with "
            "allowlisted operations only. Never pass user input to code "
            "execution functions."
        ),
        "priority": "CRITICAL",
        "effort": "Medium",
    ...
    "dangerous_code": {
        "recommendation": (
            ...
            "pickle -> json, yaml.load -> yaml.safe_load, dynamic code "
            "execution -> ast.literal_eval (for literals only). Add input "
            "validation before any dynamic operation. Implement proper error "
...
def _phase1_surface_mapping(target: Path, verbose: bool = False) -> dict:
    ...
    _entry_point_patterns = [
        re.compile(r"""(?i)(?:^main\.py|^app\.py|^server\.
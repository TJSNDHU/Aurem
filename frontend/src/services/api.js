...
/* ── TOKEN HELPERS ──────────────────────────────────────────────────────── */
export const getToken = () => sessionStorage.getItem("rr_token") || (() => { const t = localStorage.getItem("rr_token"); if (t) { sessionStorage.setItem("rr_token", t); localStorage.removeItem("rr_token"); } return t; })();
export const setToken = (t) => { sessionStorage.setItem("rr_token", t); localStorage.removeItem("rr_token"); };
export const clearToken = () => { sessionStorage.removeItem("rr_token"); localStorage.removeItem("rr_token"); };
...
export async function logout() {
  try { await req("POST", "/api/auth/logout"); } catch (e) { console.warn('Logout cleanup:', e.message); }
  clearToken();
}

...
class DocxXMLEditor(XMLEditor):
    ...
    def _convert_deltext_to_t(self, new_run):
        """Convert w:delText elements to w:t within a cloned run."""
    ...
    def _fix_run_rsid_attrs(self, new_run):
    ...
    def _revert_single_deletion(self, del_elem):
        """Create an insertion from a single w:del element's deleted content.

        Clones the deleted runs, converts w:delText back to w:t, and inserts
        a new w:ins element after the deletion.

        Args:
            del_elem: A w:del DOM element

        Returns:
            list: Nodes returned by insert_after, or None if no runs found
        """

...
class DocxXMLEditor(XMLEditor):
    ...
    def _collect_deletions(self, elem):
        """Collect w:del elements from the given element.

        Returns a tuple of (del_elements, is_single_del).
        """
        ...
        return del_elements, is_single_del

    def _create_insertion_from_deletion(self, del_elem):
        """Create a w:ins element that re-inserts the content of a w:del element.

        Clones deleted runs, converts w:delText back to w:t, and updates
        run attributes from w:rsidDel to w:rsidR.

        Args:
            del_elem: A w:del DOM element

        Returns:
            The created w:ins element, or None if del_elem has no runs.
        """

...
DIY Voice Engine Implementation

The Voice Gateway that translates voice events into AUREM's OODA format.
Uses AUREM DIY Voice Engine (WebSocket V2V + Whisper STT + OpenAI TTS).

Architecture:
1. AUREM Voice Engine -> Webhook -> voice_service.py -> Brain Orchestrator (OODA telemetry)
...
- EMERGENT_LLM_KEY: For Whisper STT + GPT-4o + OpenAI TTS
- ELEVENLABS_API_KEY: ElevenLabs voice API key (optional, for premium voices)
"""
...
# Persona system prompts for AUREM Voice Assistant
PERSONA_PROMPTS = {
...
class AuremVoiceService:
    ...
    Handles:
    1. Voice webhook processing (call events, transcripts, tool calls)
    ...
    def __init__(self, db: AsyncIOMotorDatabase):
        ...
        self._voice_engine_configured = bool(os.environ.get("EMERGENT_LLM_KEY"))
    ...
    async def process_webhook(
        ...
        """
        Main webhook handler - routes voice events to appropriate processors.
        
        This is the entry point for all voice webhooks:
        ...
        try:
            if event_type == VoiceEventType.CALL_STARTED.value:
    ...
    async def _handle_tool_call(
        ...
        This is where voice AI function calling connects to AUREM's Action Engine.
        ...
        try:
            ...
            # Return result for voice AI to continue conversation
            return {
                "status": "ok",
    ...
    async def _handle_transfer(self, business_id: str, call_id: str, payload: VoiceWebhookPayload) -> Dict:
    ...
    async def _handle_hang(self, business_id: str, call_id: str, payload: VoiceWebhookPayload) -> Dict:
    ...
    async def _handle_assistant_request(
        ...
        return {
            "assistant": None  # Uses default assistant config
        }
    ...
    async def _lookup_customer_profile(
        ...
        try:
            ...
            if customer:
                ...
                if r is not None:
                    import json
                    await r.setex(cache_key, 300, json.dumps(customer, default=str))
                return customer
...
"""
    ...
    async def initiate_outbound_call(
        ...
        """
        Initiate an outbound call via AUREM DIY Voice Engine.
        Uses the built-in V2V stream for web calls.
        """
        if not self._voice_engine_configured:
            return {
                "status": "mock",
                "message": "AUREM Voice Engine not configured (add EMERGENT_LLM_KEY). Call simulated.",
        ...
        try:
            ...
            call_id = f"out_{secrets.token_hex(8)}"
            
            if self.db is not None:
                await self.db.voice_calls.insert_one({
                    "call_id": call_id,
                    "business_id": business_id,
                    "phone_number": request.phone_number,
                    "persona": request.persona.value,
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # AUREM VOICE ASSISTANT CONFIGURATION
    # ═══════════════════════════════════════════════════════════════════════════
    
    def get_voice_assistant_config(self, persona: PersonaType, customer_tier: str = "standard") -> Dict[str, Any]:
        """
        Generate AUREM DIY Voice assistant configuration with Action Engine tools.

...
const AdminPlanManager = () => {
  // iter 315f — use the JWT minted by /admin/login (canonical helper).
  // Falls back to legacy admin_key only if no JWT in storage.
  const adminToken = getPlatformToken() || '';
  const [adminKey, setAdminKey] = useState(
    adminToken || localStorage.getItem('admin_key') || '',
  );
  const usingJwt = !!adminToken;

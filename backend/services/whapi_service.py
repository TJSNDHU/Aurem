...
import httpx
import requests  # noqa: F401 — kept for back-compat; new code uses httpx.AsyncClient
...
async def validate_whatsapp_number(phone_number: str) -> dict:
    ...
    try:
        ...
        # Use async httpx — sync requests.get inside async def blocks the whole
        # event loop for up to the timeout (was 30s!) which causes /health to
        # miss K8s probes and deployments to crash-loop.
...
async def send_whatsapp_message(phone_number: str, message: str) -> dict:
    ...
    """
    if os.environ.get("WHAPI_BLAST_DISABLED", "").lower() in ("1", "true", "yes"):
        return {"success": False, "error": "WHAPI disabled (use Twilio WABA)"}
    ...
    try:
        # ═══ CASL-compliant: wrap WhatsApp with STOP footer ═══
        try:
        ...
        # Use async httpx so a slow WHAPI response doesn't block the event loop
        # and cause /health probes to miss K8s timeout (10s).
        async with httpx.AsyncClient(timeout=10.0) as _client:

...
class PolygonWalletService:
    ...
    async def create_treasury_wallet(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Encrypt the private key BEFORE persisting. AES/Fernet-based
            # symmetric encryption keyed on WALLET_ENCRYPTION_KEY (falls
            # back to JWT-derived key when env missing). See
            # services/crypto_treasury/wallet_crypto.py
            from services.crypto_treasury.wallet_crypto import encrypt as _enc_key
            encrypted_pk = _enc_key(account.key.hex())

            wallet_info = {
                "wallet_id": f"treasury_{secrets.token_hex(8)}",
                "address": account.address,
                "private_key": encrypted_pk,  # ENCRYPTED (fernet:v1:<token>)
                "key_encryption": "fernet-v1",
                "network": "polygon",
    ...
    async def send_usdt(
        ...
        try:
            ...
            if not private_key:
                ...
                # Decrypt if stored under the fernet:v1 envelope; legacy
                # plaintext rows are returned as-is by `decrypt` for
                # backward compatibility. Run
                # `migrate_plaintext_to_encrypted` to upgrade old rows.
                try:
                    from services.crypto_treasury.wallet_crypto import decrypt as _dec_key
                    private_key = _dec_key(private_key)
                except Exception as _de:
                    logger.error(f"[Polygon] private_key decrypt failed: {_de}")
                    raise

...
API endpoints for AUREM DIY voice AI integration

Endpoints:
- POST /api/voice/webhook - Voice event handler (main entry point)
- GET /api/voice/webhook - Webhook verification
...
- GET /api/voice/{business_id}/config/{persona} - Get assistant config
...
async def verify_webhook(
    request: Request,
    token: Optional[str] = Query(None, alias="verify-token")
):
    """
    Voice webhook verification endpoint.
    Verifies the webhook URL during setup.
    """
    import os
    
    expected_token = os.environ.get("VOICE_WEBHOOK_SECRET", "aurem-voice-webhook")
...
async def handle_webhook(
    request: Request,
    x_voice_signature: Optional[str] = Header(None, alias="x-voice-signature")
):
    """
    Main AUREM DIY voice webhook handler.
    ...
    try:
        payload = VoiceWebhookPayload(**raw_body)
    except Exception as e:
        logger.warning(f"[Voice] Payload parsing failed: {e}")
        payload = VoiceWebhookPayload(type=raw_body.get("type", "unknown"))
...
async def get_assistant_config(
    ...
    """
    Get AUREM voice assistant configuration for a persona.
    
    Use this to create/update voice assistants with AUREM's
    ...
    voice_service = get_voice_service(get_db())
    config = voice_service.get_voice_assistant_config(persona_type, customer_tier=effective_tier)
    
    # Replace placeholders with actual values
    import os
    webhook_url = os.environ.get("VOICE_WEBHOOK_URL", f"{request.base_url}api/voice/webhook/{business_id}")
...
async def health():
    ...
    return {
        ...
        "configuration": {
            "emergent_llm_key": "configured" if emergent_key_configured else "not_configured",
            ...
            "stt": "whisper-1 (via Emergent Key)" if emergent_key_configured else "browser_web_speech_api",
            "tts": "openai_tts-1 (via Emergent Key)" if emergent_key_configured else "browser_speech_synthesis",
            "llm": "gpt-4o (via Emergent Key)" if emergent_key_configured else "not_configured",
        },
        "mode": "v2v_stream" if emergent_key_configured else "browser_fallback",
        "capabilities": [
            "websocket_v2v_stream",
            "real_time_stt_whisper",
            "real_time_tts_openai",
            "tone_sync_sentiment",
...
# ==================== TOOL DEFINITIONS FOR AUREM VOICE AI ====================

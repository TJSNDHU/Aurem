...
async def send_whatsapp(phone: str, message: str) -> Dict:
    ...
    # If we hit a 401 recently, the channel auth is expired. Skip sending
    # for 1h to prevent log-flood; ORA bell already got the notification.
    try:
        from utils.redis_pool import get_async_redis
        _r = await get_async_redis()
        if _r is not None:
            try:
                flag = await _r.get("whatsapp:auth_circuit_open")
                if flag:
                    return {"success": False, "skipped": True, "reason": "auth_circuit_open"}
            except Exception:
                pass
    except Exception:
        pass
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                # ── Auth-expired path — open the circuit for 1 h ─────
                if response.status_code == 401:
                    await _open_whatsapp_auth_circuit(response.text)
                    return {"success": False, "error": "auth_expired_circuit_opened"}
...
async def _open_whatsapp_auth_circuit(err_body: str):
    """Open the 1h auth circuit and record the alert once.

    Suppresses the 401-per-message log flood until the channel is fixed.
    """
    try:
        from utils.redis_pool import get_async_redis
        _r = await get_async_redis()
        if _r is not None:
            # Only log ALERT on first open (use SET NX with 1h TTL)
            try:
                was_set = await _r.set("whatsapp:auth_circuit_open", "1", ex=3600, nx=True)
                if was_set and _db is not None:
                    try:
                        await _db.system_alerts.insert_one({
                            "type": "whatsapp_auth_expired",
                            "action": "circuit_open_1hr",
                            "error": (err_body or "")[:300],
                            "timestamp": datetime.now(timezone.utc),
                        })
                        logger.warning("[WhatsApp] AUTH EXPIRED — circuit opened for 1 h; ORA bell notified")
                    except Exception:
                        pass
                    # Best-effort ORA bell push (founder_notifications collection)
                    try:
                        await _db.founder_notifications.insert_one({
                            "type": "whatsapp_auth_expired",
                            "title": "WhatsApp channel auth expired",
                            "body": "Outbound WhatsApp sends paused for 1 h. Re-authorize the WHAPI channel.",
                            "severity": "warn",
                            "timestamp": datetime.now(timezone.utc),
                        })
                    except Exception:

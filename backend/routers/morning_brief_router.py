...
async def fetch_weather(location: str = DEFAULT_LOCATION) -> Dict[str, Any]:
    """
    Fetch current weather data via Open-Meteo (free, no API key needed).
    Falls back to mock data if unavailable.
    """
    try:
        from services.free_api_arsenal import get_weather
        w = await get_weather(43.59, -79.65, location.split(",")[0] if "," in location else location)
        if w.get("temp_c") is not None:
            return {
                "temperature": round(w["temp_c"]),
                "feels_like": round(w.get("feels_like_c", w["temp_c"])),
                "condition": w.get("condition", "Clear"),
                "description": w.get("condition", "clear sky"),
                "humidity": w.get("humidity", 50),
                "wind_speed": round(w.get("wind_kph", 0)),
                "location": w.get("city", location),
                "icon": "01d",
                "source": "open-meteo (free)",
            }
    except Exception as e:
        logger.warning(f"[MorningBrief] Open-Meteo failed: {e}")
...
async def get_morning_brief(
    ...
    # Sentinel Health + Revenue Forecast + Learning Velocity injections
    ...
    try:
        db = get_db()
        if db is not None:
            recent_anomalies = await db.sentinel_diagnoses.find(
                {"type": {"$in": ["performance_anomaly", "anomaly"]}},
                {"_id": 0, "stage": 1, "severity": 1, "elapsed_ms": 1}
            ).sort("created_at", -1).limit(5).to_list(5)

            pulse = await db.system_pulse.find_one({}, {"_id": 0, "health_score": 1})
            h_score = pulse.get("health_score", "N/A") if pulse else "N/A"
            sentinel_health = f"System health: {h_score}/100 | {len(recent_anomalies)} recent anomalies"
    except Exception:
        pass

    try:
        from services.revenue_forecast import get_morning_brief_line
        revenue_forecast = await get_morning_brief_line(business_id)
    except Exception:
        pass

    try:
        from services.memory_tiers import get_learning_velocity
        velocity = await get_learning_velocity(business_id)
        score = velocity.get("compound_score", 0)
        learning_velocity = f"Learning velocity: {score}/100"
    except Exception:
        pass
    ...
    return {
        ...
        "phases": {
            ...
            "phase_2": {
                ...
                "components": ["AUREM DIY Voice Engine", "WhatsApp Cloud API", "Gmail Integration", "Unified Inbox"]
            },
            "phase_3": {
                "name": "Action Engine",
        ...
        "roadmap": {
            "p0": ["Configure AUREM DIY Voice Engine", "Configure Exa API for Reddit"],
            ...
            "stages": [
                {
                    ...
                    "channels": ["AUREM DIY Voice", "WhatsApp", "Email", "Web Chat"],

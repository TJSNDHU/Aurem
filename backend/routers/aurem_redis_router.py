...
async def websocket_endpoint(websocket: WebSocket, business_id: str, token: str = None):
    """WebSocket endpoint for real-time dashboard updates.
    business_id in URL must match the tenant_id in the JWT."""

    # ── AUTH GATE: Validate JWT ──
    if not token:
        await websocket.close(code=4001, reason="Missing authentication token")
        return
    try:
        payload = pyjwt.decode(token, JWT_SECRET, algorithms=[JWT_ALGORITHM])
        jwt_tenant = payload.get("tenant_id", payload.get("user_id"))
    except pyjwt.InvalidTokenError:
        await websocket.close(code=4001, reason="Invalid token")
        return

    # ── TENANT ISOLATION: JWT tenant must match URL business_id ──
    if jwt_tenant != business_id:

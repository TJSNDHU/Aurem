...
class SkillsManager:
    ...
    def __init__(self):
        ...
        # SuperSkills registry (Keshav Sharma's 77 agent-specific prompts)
        try:
            from .superskills_loader import registry_stats
            ss_stats = registry_stats()
            logger.info(
                f"[SkillsManager] SuperSkills loaded: {ss_stats['total']} skills "
                f"({', '.join(f'{k}={v}' for k,v in ss_stats['by_agent'].items())})"
            )
        except Exception as e:
            logger.warning(f"[SkillsManager] SuperSkills load failed: {e}")

        logger.info(f"[SkillsManager] Initialized with {len(self.skills)} core skills")
    ...
    def list_superskills(
        self, agent: Optional[str] = None, category: Optional[str] = None
    ) -> Dict[str, Any]:
        """List all SuperSkills manifests (from Keshav's superskills repo)."""
        try:
            from .superskills_loader import list_skills, registry_stats
            return {
                "stats": registry_stats(),
                "skills": list_skills(agent=agent, category=category),
            }
        except Exception as e:
            return {"error": str(e), "stats": {"total": 0}, "skills": []}

    def match_superskills(
        self, query: str, agent: str = "claude", limit: int = 3
    ) -> list:
        """Find SuperSkills matching a query (for prompt augmentation)."""
        try:
            from .superskills_loader import match_skills
            return match_skills(query, agent=agent, limit=limit)
        except Exception:
            return []

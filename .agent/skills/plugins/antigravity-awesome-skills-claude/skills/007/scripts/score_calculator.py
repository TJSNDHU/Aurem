...
def _score_compliance_domain(scores: dict[str, float]) -> float:
    """Compute the compliance domain score as an aggregate of other scores."""
    other_scores = [
...
def compute_domain_scores(
    ...
    scores["authn_authz"] = _score_authn_authz_domain(source_files, total_source_files)
    scores["data_protection"] = _score_data_protection_domain(
        secrets_findings, source_files, total_source_files,
    )
    scores["resilience"] = _score_resilience_domain(source_files, total_source_files)
    scores["monitoring"] = _score_monitoring_domain(source_files, total_source_files)

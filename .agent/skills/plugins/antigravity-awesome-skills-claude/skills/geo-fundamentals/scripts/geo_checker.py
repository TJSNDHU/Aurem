...
def _check_structured_data(content: str, issues: list, passed: list):
    """Check for JSON-LD structured data (Critical for AI)."""
...
def _check_headings(content: str, issues: list, passed: list):
...
def _check_author(content: str, issues: list, passed: list):
...
def _check_date(content: str, issues: list, passed: list):
...
def _check_entity(content: str, passed: list):
    """Check for entity recognition (E-E-A-T signal) - NEW 2025."""
    entity_patterns = [
...
def _check_statistics(content: str, passed: list):
    """Check for original statistics/data (AI citation magnet) - NEW 2025."""
    stat_patterns = [
        r'\d+%',                    # Percentages
        r'\$[\d,]+',                # Dollar amounts
...
def _check_direct_answers(content: str, passed: list):
    """Check for conversational/direct answers - NEW 2025."""
...
def check_page(file_path: Path) -> dict:
    ...
    _check_structured_data(content, issues, passed)
    _check_headings(content, issues, passed)
    _check_author(content, issues, passed)
    _check_date(content, issues, passed)

...
class BaseSchemaValidator:
    ...
    def validate_unique_ids(self):
        ...
        for xml_file in self.xml_files:
            self._validate_unique_ids_in_file(xml_file, global_ids, errors)
    ...
    def _validate_unique_ids_in_file(self, xml_file, global_ids, errors):
        """Validate ID uniqueness within a single XML file."""
        try:
            root = lxml.etree.parse(str(xml_file)).getroot()
            file_ids = {}  # Track IDs that must be unique within this file

            self._strip_alternate_content(root)
            ...
            for elem in root.iter():
                ...
                id_value = self._get_id_attribute(elem, attr_name)
                if id_value is None:
                    continue
                self._check_id_uniqueness(
                    elem, tag, attr_name, scope, id_value,
                    xml_file, file_ids, global_ids, errors,
                )
    ...
    def _strip_alternate_content(self, root):
    ...
    def _get_local_tag(elem):
        """Return the element's local name (without namespace), lowercased."""
        return (
    ...
    @staticmethod
    def _get_id_attribute(elem, attr_name):
        """Look for the specified attribute on elem and return its value, or None."""
    ...
    def _check_id_uniqueness(
        self, elem, tag, attr_name, scope, id_value,
        xml_file, file_ids, global_ids, errors,
    ):
        """Check a single ID value for uniqueness and append errors if duplicated."""
        if scope == "global":
            self._check_global_id(
                id_value, tag, elem, xml_file, global_ids, errors
            )
        elif scope == "file":
            self._check_file_id(
                id_value, tag, attr_name, elem, xml_file, file_ids, errors
            )

    def _check_global_id(self, id_value, tag, elem, xml_file, global_ids, errors):
        """Check that an ID is globally unique across all files."""
    ...
    def _check_file_id(self, id_value, tag, attr_name, elem, xml_file, file_ids, errors):
        """Check that an ID is unique within the current file."""
        key = (tag, attr_name)

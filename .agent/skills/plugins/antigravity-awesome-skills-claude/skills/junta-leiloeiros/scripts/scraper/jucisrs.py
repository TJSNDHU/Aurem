...
class JucisrsScraper(AbstractJuntaScraper):
    ...
    def _extract_matricula_nome(self, lines: List[str]):
        ...
        Retorna (nome, matricula, situacao, remaining_lines) ou (None, None, None, []).
        """
        for i, line in enumerate(lines):
            ...
            if m:
                ...
                return nome, matricula, situacao, lines[i+1:]
            ...
            if line.isdigit() and i + 1 < len(lines):
                ...
                if next_line.startswith("- ") or next_line.startswith("– "):
                    ...
                    if RE_CANCELADO.search(nome_raw):
                        ...
                        nome_raw = RE_CANCELADO.sub("", nome_raw).strip(" ")
                    nome = self.clean(nome_raw)
                    return nome, matricula, situacao, lines[i+2:]
        return None, None, None, []

    def _parse_remaining_lines(self, record: dict, remaining: List[str]) -> None:
        """Preenche os campos do record a partir das linhas restantes do bloco."""
    ...
    def _parse_block(self, block: str) -> dict | None:
        """Parseia um unico bloco de HTML (uma entrada de leiloeiro)."""
        ...
        nome, matricula, situacao, remaining = self._extract_matricula_nome(lines)
        ...
        self._parse_remaining_lines(record, remaining)
        return record

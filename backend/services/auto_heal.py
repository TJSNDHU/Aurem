...
def _get_db():
    """Get database reference, self-connecting from env if needed."""
    ...
    try:
        ...
        if not mongo_url:
            logger.warning("[AUTO_HEAL] No MONGO_URL — MongoDB check will be skipped")
            return None
        from motor.motor_asyncio import AsyncIOMotorClient
        client = AsyncIOMotorClient(mongo_url)
        db_name = os.environ.get("DB_NAME", "aurem_db")
        _db = client[db_name]
        logger.info(f"[AUTO_HEAL] Self-connected to MongoDB: {db_name}")
        return _db
    except Exception as e:
        logger.error(f"[AUTO_HEAL] Self-connect to MongoDB failed: {e}")
        return None

# Admin phone for alerts (Tj's number)
ADMIN_WHATSAPP = os.environ.get("ADMIN_WHATSAPP", "+16134000000")

# Backend URL - use localhost in preview, SITE_URL in production
BACKEND_URL = "http://localhost:8001"  # always local — auto-heal runs inside the pod
...
def set_db(database):
    """Set database reference for auto_heal module (called by startup_init)"""
    global _db, _db_init_attempted
    _db = database
    _db_init_attempted = True
...
async def log_auto_heal_action(
    ...
    try:
        ...
        await _get_db().auto_heal_log.insert_one(log_entry)
...
async def check_redis_connection() -> Dict[str, Any]:
    ...
    try:
        ...
        if redis_client is None:
            return {"check": check_name, "status": "skipped", "reason": "Redis pool unavailable"}

        # Ping Redis (shared pool — do NOT close)
...
async def check_ai_chat_working() -> Dict[str, Any]:
    ...
    NOTE: Uses 5s timeout (not 20s) — when OpenRouter is rate-limited the AI
    chat endpoint can take 10-30s which starves the single uvicorn worker.
    A 5s timeout gives us a quick signal without blocking other requests.
    """
    check_name = "ai_chat"
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code == 200 and ("reply" in data or "response" in data):
                ...
                # Rate limited — this is not a service failure, just throttling
                return {
                    "check": check_name,
                    "status": "healthy",
                    "note": "Rate limited (429) — service is up but throttled",
                    "action_taken": None
                }
            else:
                # AI chat is broken!
        ...
        # Timeout is NOT a bug — often just rate-limiting cascade.
        # Don't page the admin over it.
        logger.warning(f"[AUTO_HEAL] ai_chat timed out (5s) — probably rate-limit cascade, skipping alert")
        return {
            "check": check_name,
            "status": "skipped",
            "note": "Timed out at 5s — non-critical, likely rate-limit cascade",
            "action_taken": None,
        }
...
async def check_error_logs() -> Dict[str, Any]:
    ...
    try:
        ...
        if _get_db() is not None:
            ...
            cursor = _get_db().error_logs.find({
...
async def run_all_health_checks() -> Dict[str, Any]:
    ...
    if _get_db() is not None:
        try:
            await _get_db().auto_heal_runs.insert_one({
        ...
        # Heartbeat write to auto_heal_log so the pillars_map monitor can detect
        # p4_command_hub liveness even when no issue was found this cycle.
        # Without this, healthy platforms trip the "300s stale" drift alarm.
        try:
            await _get_db().auto_heal_log.insert_one({
                "timestamp": results["timestamp"],
            ...
            logger.warning(f"[AUTO_HEAL] Failed to write heartbeat: {e}")
...
async def auto_heal_scheduler():
    ...
    """
    logger.info("[AUTO_HEAL] Auto-heal scheduler started (first run in 5 min, then every 10 min)")
    
    # Wait 5 minutes after startup — lets OpenRouter 429 / Emergent 502 cascades
    # clear before auto_heal starts making self-HTTP calls that can starve the
    ...
    while True:
        try:
            ...
            # ── Resource Guardian — proactive OOM prevention ────────
            try:
                from services.resource_guardian import check_and_heal_resources
                from utils.redis_pool import get_async_redis
                _r = await get_async_redis()
                await check_and_heal_resources(_get_db(), _r, task_registry={})
            except Exception as e:
                logger.warning(f"[AUTO_HEAL] resource_guardian error: {e}")

            # Wait 10 minutes before next check — prevents overlapping runs
            # when individual runs exceed 5 minutes (e.g., OpenRouter rate-limit chains).
...
async def get_auto_heal_logs(limit: int = 50) -> list:
    ...
    try:
        logs = await _get_db().auto_heal_log.find(
            {},
            {"_id": 0}

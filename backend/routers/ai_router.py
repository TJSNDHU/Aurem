...
try:
    ...
    _llm_chat_available = bool(EMERGENT_LLM_KEY)
except ImportError:
    pass


async def _emergent_fallback(prompt: str, system: str = None, max_tokens: int = 1024) -> str:
    """Fallback to Emergent LLM (Gemini) when OpenRouter is unavailable."""
    if not _llm_chat_available:
        raise HTTPException(status_code=503, detail="No AI provider configured")
    import secrets
    sys_msg = system or "You are a helpful AI assistant for the AUREM platform."
    chat = LlmChat(
        api_key=EMERGENT_LLM_KEY,
        session_id=f"fallback_{secrets.token_hex(6)}",
...
MODEL_ROUTING = {
    ...
    "skin_advisor": {"provider": "anthropic", "model": "claude-sonnet-4-5-20250929"},
    "whatsapp_twin": {"provider": "anthropic", "model": "claude-sonnet-4-5-20250929"},
    "formulation": {"provider": "anthropic", "model": "claude-sonnet-4-5-20250929"},
...
async def call_openrouter(prompt: str, model: str, max_tokens: int = 1024, temperature: float = 0.7, system: str = None) -> str:
    ...
    if not OPENROUTER_API_KEY:
        return await _emergent_fallback(prompt, system, max_tokens)

    # Process-wide kill-switch: once the key is proven invalid (401), skip
    # OpenRouter for the rest of the pod's lifetime. Prevents log spam on
    # every request + every retry + every fallback model.
    if getattr(call_openrouter, '_dead', False):
        return await _emergent_fallback(prompt, system, max_tokens)

    # Skip OpenRouter entirely if key was recently rejected (1 hour cooldown)
    import time as _t
    global _or_key_bad_until
    if hasattr(call_openrouter, '_bad_until') and _t.time() < call_openrouter._bad_until:
        return await _emergent_fallback(prompt, system, max_tokens)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "choices" in data and len(data["choices"]) > 0:
                        ...
                        if content:
                            return content
                    logger.warning(f"OpenRouter empty response from {try_model}")
                    last_error = "Empty response"
                    continue
                elif response.status_code == 401:
                    # Permanent kill-switch — key is bad, don't retry for this process lifetime.
                    # Logs ONCE total, not on every call.
                    if not getattr(call_openrouter, '_dead', False):
                        logger.warning(f"OpenRouter 401 on {try_model} — disabling for this process, using Emergent fallback (graceful).")
                        call_openrouter._dead = True
                    call_openrouter._bad_until = time.time() + 3600
                    return await _emergent_fallback(prompt, system, max_tokens)
                elif response.status_code == 429:
                    # Early exit on rate-limit — trying more free-tier models will
                    # most likely hit 429 too, and each try costs 10s on event loop.
                    # Log only once per model per pod to prevent deploy-log spam.
                    if not hasattr(call_openrouter, '_logged_429'):
                        call_openrouter._logged_429 = set()
                    if try_model not in call_openrouter._logged_429:
                        logger.info(f"OpenRouter 429 on {try_model} — free-tier saturated, going to Emergent")
                        call_openrouter._logged_429.add(try_model)
    ...
    # All models failed — fall back to Emergent instead of crashing
    logger.warning(f"All OpenRouter models failed, using Emergent fallback. Last error: {last_error}")
    return await _emergent_fallback(prompt, system, max_tokens)

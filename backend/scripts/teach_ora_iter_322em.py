"""
teach_ora_iter_322em — Hard-save Execution Layer skill into ORA memory.
"""
...
SKILL = {
    "iter":     "322em",
    "id":       "aurem-322em-ora-execution-layer",
    "name":     "ORA Execution Layer — Python Subprocess Bridge to Linux Kernel",
    "category": "execution_tools",
    "description": (
        "P2 execution capability — real subprocess.run() against the Linux "
        "kernel via shell_exec tool. NOT a mock. Whitelisted commands: "
        "whoami, id, pwd, hostname, uname, uptime, date, env (redacted), "
        "ls, find, wc, stat, du, file, df, free, ps, which, whereis, "
        "python3/node/pip/yarn versions, ruff, git read-only verbs, "
        "supervisorctl status. shell=False, argv-only, stripped env, hard "
        "timeouts. Every call audit-logged."
    ),
    "md_path":  "/app/backend/ora_skills/dev_ora-execution-layer.md",
}
...
async def main() -> None:
    ...
    print(f"=== iter 322em — Hard-saving Execution Layer skill ===")
    ...
    await db.ora_training_files.update_one(
        ...
        {"$set": {
            ...
            "filename":     f"learning-{SKILL['iter']}-execution-layer.md",
            "file_ext":     ".md",
    ...
    # 3. broadcast — Charter stays first, execution layer goes right after
    ...
    if SKILL["id"] in existing_ids:
        existing_ids.remove(SKILL["id"])
    # Insert at position 1 (right after Charter@0)
    if existing_ids and "zero-hallucination" in existing_ids[0]:
        final_ids = [existing_ids[0], SKILL["id"]] + existing_ids[1:]
    else:
    ...
    await db.ora_skills_broadcast_history.insert_one({
        "_id":            f"bcast_{now}_322em",
        "skill_ids":      final_ids,
        "target_agents":  "ALL",
        "broadcast_at":   now,
        "broadcast_by":   "main_agent_iter_322em",
        "action":         "broadcast_with_execution_layer",
    })
    try:
    ...
    print(f"  PRIMARY  broadcast:       {bcast.get('skill_count')} total skills, "
          f"contains exec layer = {SKILL['id'] in (bcast.get('skill_ids') or [])}")

...
const SalesPipelineDashboard = ({ token }) => {
  ...
  const fetchPredictions = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/intelligence/pipeline/predictions`, {
  ...
  }, [token, fetchPredictions, fetchRecentScans]);
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-white/60 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-light text-[#1A1A2E] tracking-wider mb-2">Sales Pipeline</h1>
        ...
        {/* Pipeline Steps */}
        <div className="mb-8 p-6 bg-white/80 backdrop-blur-sm border border-[#FF6B00]/20 rounded-lg">
          ...
          <div className="mb-8 p-5 rounded-xl" style={{ background: 'var(--aurem-card-bg)', border: '1px solid var(--aurem-border)' }} data-testid="pipeline-predictions">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-4 text-[10px]">
                <span style={{ color: 'var(--aurem-body-secondary)' }}>Pipeline: <strong style={{ color: '#4ade80' }}>${(predictions.summary?.total_pipeline_value || 0).toLocaleString()}</strong></span>
                <span style={{ color: 'var(--aurem-body-secondary)' }}>Weighted: <strong style={{ color: '#D4AF37' }}>${(predictions.summary?.weighted_pipeline_value || 0).toLocaleString()}</strong></span>
              </div>
            </div>
            <div className="space-y-2">
              {predictions.predictions.slice(0, 6).map((deal, i) => (
                <div key={i} className="flex items-center gap-4 p-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--aurem-border)' }}>
                  <div className="flex-1 min-w-0">
                    <div className="text-xs font-medium truncate" style={{ color: 'var(--aurem-heading)' }}>{deal.title}</div>
                    <div className="text-[10px]" style={{ color: 'var(--aurem-body-secondary)' }}>{deal.company} | {deal.stage}</div>
                  </div>
                  <div className="text-right text-xs font-mono" style={{ color: '#4ade80' }}>${(deal.value || 0).toLocaleString()}</div>
                  <div className="w-24">
                    <div className="flex items-center gap-1.5">
                      <div className="flex-1 h-1.5 rounded-full" style={{ background: 'var(--aurem-border)' }}>
                        <div className="h-full rounded-full transition-all" style={{
                          width: `${(deal.prediction?.win_probability || 0) * 100}%`,
                          background: deal.prediction?.win_probability > 0.6 ? '#4ade80' : deal.prediction?.win_probability > 0.3 ? '#f59e0b' : '#f87171'
                        }} />
                      </div>
                      <span className="text-[10px] font-mono w-8 text-right" style={{
                        color: deal.prediction?.win_probability > 0.6 ? '#4ade80' : deal.prediction?.win_probability > 0.3 ? '#f59e0b' : '#f87171'
                      }}>{Math.round((deal.prediction?.win_probability || 0) * 100)}%</span>
                    </div>
                  </div>
                  <div className="text-[9px] px-2 py-0.5 rounded-full" style={{
                    background: deal.prediction?.deal_health === 'healthy' ? 'rgba(74,222,128,0.1)' : deal.prediction?.deal_health === 'at_risk' ? 'rgba(245,158,11,0.1)' : 'rgba(248,113,113,0.1)',
                    color: deal.prediction?.deal_health === 'healthy' ? '#4ade80' : deal.prediction?.deal_health === 'at_risk' ? '#f59e0b' : '#f87171',
                  }}>{deal.prediction?.deal_health || 'unknown'}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {recentScans.slice(0, 6).map((scan) => (
                  <div
                    ...
                    className="p-4 bg-white/60 border border-[#FF6B00]/20 rounded-lg cursor-pointer hover:border-[#D4AF37]/30 transition-all"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <h3 className="text-sm font-medium text-[#1A1A2E] mb-1">{scan.website_url}</h3>
              ...
              {decisionMakers.map((dm, idx) => (
                <div key={idx} className="p-4 bg-white/60 border border-[#FF6B00]/20 rounded-lg flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-[#1A1A2E]">{dm.name}</h3>
        ...
        {currentStep >= 3 && proposal && (
          <div className="p-6 bg-white/80 backdrop-blur-sm border border-[#FF6B00]/20 rounded-lg mb-6">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="p-4 bg-white/60 border border-[#FF6B00]/20 rounded-lg">
              ...
              <div className="p-4 bg-white/60 border border-[#FF6B00]/20 rounded-lg">
                ...
                <div className="text-2xl font-bold text-[#1A1A2E]">${proposal.pricing?.setup_fee}</div>
              </div>
              <div className="p-4 bg-white/60 border border-[#FF6B00]/20 rounded-lg">
          ...
          <div className="p-6 bg-gradient-to-br from-[#D4AF37]/10 to-[#8B7355]/10 border border-[#D4AF37]/30 rounded-lg">
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-[#1A1A2E]">Progress</span>

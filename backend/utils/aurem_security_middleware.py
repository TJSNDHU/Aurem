...
class AuremRateLimitMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if auth.startswith("Bearer "):
            # Decode JWT to get stable user identity — same verifier used
            # by the rest of the platform so rate-limit buckets are keyed
            # on a real user, not a rolling token prefix.
            try:
                ...
                # Invalid/expired token → treat as anonymous and let
                # the rate limiter bucket by IP instead of faking a user.
                user_id = None

...
async def get_connection_status():
    """Returns whether Coinbase is in mock or live mode, for frontend mock/live badge."""
    if _db is None:
        return {"mode": "unavailable", "message": "Database not initialized"}
    try:
        from services.crypto_treasury.coinbase_service import get_coinbase_service
        svc = get_coinbase_service(_db)
        await svc.load_api_keys()
        return {
            "mode": "live" if not svc.mock_mode else "mock",
            "mock_mode": svc.mock_mode,
            "message": "Coinbase API keys configured" if not svc.mock_mode else "Running in mock mode — add Coinbase API keys via Admin to go live",
        }
    except Exception as e:
        return {"mode": "mock", "mock_mode": True, "message": str(e)}

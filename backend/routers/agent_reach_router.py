...
async def crawler_status():
    """
    Get status of the hourly background crawler.
    Shows last crawl timestamp, docs processed, and next scheduled run.
    """
    db = get_db()
    last_log = await db.crawler_logs.find_one(
        {},
    ...
    return {
        "crawler": "hourly_agent_reach",
        "schedule": "every 60 minutes",
        "last_crawl": last_log if last_log else "no_crawls_yet",
        "vector_db": vs_stats,
...
async def trigger_crawl():
    """
    Manually trigger a crawl cycle (does not wait for hourly schedule).
    """
    import asyncio
    from services.agent_reach_service import _run_crawl_cycle
    ...
    try:
        ...
        return {"status": "completed", "result": last_log}
...
async def health():
    ...
    return {
        ...
        "configuration": {
            ...
            "embedding_model": "all-MiniLM-L6-v2 (local, 384d)"
        },

...
class AdminTools:
    ...
    async def send_whatsapp_to_customer(
        ...
        try:
            ...
            from_number = get_twilio_whatsapp_from()
            if not from_number:
                return {"success": False, "error": "Twilio WhatsApp number not configured (set TWILIO_PHONE_NUMBER or TWILIO_WHATSAPP_NUMBER)"}
            # channel_config returns 'whatsapp:<number>' already prefixed
            from_param = from_number
            to_param = f"whatsapp:{phone}" if not phone.startswith("whatsapp:") else phone
